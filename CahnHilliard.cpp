...
{
    MFEM_VERIFY(mobility_ != nullptr, "Mobility must be set before BuildMatricies()");
...
void CahnHilliardOperator::Mult(const mfem::Vector &x, mfem::Vector &y) const 
{
    M_phi_.Mult(x, y);
}

const mfem::SparseMatrix &CahnHilliardOperator::GetMass()    const { return M_phi_; }
const mfem::SparseMatrix &CahnHilliardOperator::GetKmob()    const { return K_mob_; }